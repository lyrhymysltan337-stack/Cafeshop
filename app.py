...
# ---------------------------------------------------------------------------
# منوی پیش‌فرض: اگر فایل menu_data.json وجود نداشته باشد (مثلاً بعد از یک
# دیپلوی تازه روی هاست)، این داده‌ها به‌طور خودکار استفاده و روی دیسک ذخیره
# می‌شوند تا از همان لحظه‌ی اول محصولات توی سایت باشند. بعد از آن، ویرایش/
# اضافه/حذف محصولات از همان پنل مدیریت مثل قبل کار می‌کند.
# ---------------------------------------------------------------------------
DEFAULT_DATA_JSON = r'''
{
  "categories": [
  ...
  "password": null,
  "tagline": "Be kind with your taste"
}
'''


def _default_data():
    return json.loads(DEFAULT_DATA_JSON)
